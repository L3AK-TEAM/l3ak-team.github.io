---
author: Matthias and Suvoni
cat: misc & crypto
date: 2024-04-21
title: DHCPPP
event: Plaid CTF 2024
chal: DHCPPP
blurb: Exploiting ChaCha20-Poly1305 nonce reuse to forge DHCP configuration, poison DNS, and exfiltrate the flag.
---
...
We also have some functions to encrypt and decrypt data using the **ChaCha20\_Poly1305** algorithm, which uses a `key` and a `nonce` to encrypt a `message` and produces a `ciphertext` and an authentication `tag` (a.k.a. message authentication code or MAC for short - not to be confused with a *MAC address* though).
...
- Typically you’re able to reuse the key for ChaCha20\_Poly1305, as long as you’re using a different nonce for each message encryption; otherwise this algorithm is known to lose confidentiality for messages encrypted using the same nonce.
...
- Both the `encrypt_msg` and the `decrypt_msg` functions use the same static key `CHACHA_KEY` for the **ChaCha20\_Poly1305** cipher.
...
$$
\operatorname{tag}_1
= \left(\operatorname{Poly1305}_r(\operatorname{msg}_1) + s\right) \bmod 2^{128}
\tag{1}
$$

$$
\operatorname{tag}_2
= \left(\operatorname{Poly1305}_r(\operatorname{msg}_2) + s\right) \bmod 2^{128}
\tag{2}
$$
...
$$
\operatorname{tag}_1 - \operatorname{tag}_2
\equiv
\operatorname{Poly1305}_r(\operatorname{msg}_1)
- \operatorname{Poly1305}_r(\operatorname{msg}_2)
\pmod{2^{128}}
$$

Expanding the Poly1305 polynomials:

$$
\begin{aligned}
\operatorname{tag}_1 - \operatorname{tag}_2
&\equiv
\Bigl(c_1^1 r^q + c_1^2 r^{q-1} + \cdots + c_1^q r\Bigr) \bmod (2^{130}-5) \\
&\quad -
\Bigl(c_2^1 r^q + c_2^2 r^{q-1} + \cdots + c_2^q r\Bigr) \bmod (2^{130}-5)
\pmod{2^{128}}.
\end{aligned}
$$

Which can be rewritten over the Poly1305 field as:

$$
\operatorname{tag}_1 - \operatorname{tag}_2 + 2^{128}k
\equiv
(c_1^1-c_2^1)r^q
+ (c_1^2-c_2^2)r^{q-1}
+ \cdots
...
- [PyCryptodome ChaCha20\_Poly1305.py](https://github.com/Legrandin/pycryptodome/blob/master/lib/Crypto/Cipher/ChaCha20_Poly1305.py)
